...
function a3(){
	...
	window.location.href ="showAllBX.do?size="+s;
}
...
$('li.active').parents('li').addClass('treeview').addClass('active');
	...
	<tbody>
		<tr>
			<th class="hidden-xs ext-center" style="width:80px">来源公司</th>
			<th class="hidden-xs ext-center" style="width:80px">操作人姓名</th>
			<th class="hidden-xs ext-center" style="width:80px">金额</th>
			<th class="hidden-xs ext-center" style="width:100px">查询时间</th>
			<th class="hidden-xs ext-center" style="width:100px">结束时间</th>
			<th style="width: 120px" class="text-center">类型</th>
			<th style="width: 80px" class="text-center">查询状态</th>		
		</tr>
			<c:forEach items="${requestScope.list}" var="list" >										
			<tr>
			<td class="hidden-xs">${list.api_companyname}</td>
			<td class="hidden-xs">${list.api_name}</td>
			<td class="hidden-xs">-35.00元</td>
			<td class="hidden-xs">${list.dt_add}</td>
			<td class="hidden-xs">${list.dt_edit}</td>
			<td class="text-center">		
			<span class="label label-danger">车辆出险查询</span>			
			</td>
			<td class="text-center">
				<c:choose>
					<c:when test="${list.bc_status==3}">
						<span class="label label-success">查询完成</span>
					</c:when>
					<c:otherwise>
						<span class="label label-danger">正在查询</span>
					</c:otherwise>
				</c:choose>
			</td>
		    </tr>
...
<div class="foot-page">
				...
				<ul class="pagination no-margin">
				       ...
				       <li >
				       <a  href="showAllBX.do?sa=${requestScope.sa}&size=${requestScope.size }&pagenow=${requestScope.pagenow-1}">
				       <span >«</span>
				       </a>
				       </li>
				       </c:if>
					   ...
					   <a id="a<%=j %>" href="showAllBX.do?sa=${requestScope.sa}&size=${requestScope.size }&pagenow=<%=j %>">
					   <%=j %>
					   </a>
					   </li>
					   <%
					   ...
					   %>
	                   <li id="l<%=j %>" >
					   <a id="a<%=j %>" href="showAllBX.do?sa=${requestScope.sa}&size=${requestScope.size }&pagenow=<%=j %>"><%=j %>					   
					   </a>
					   </li>					   
		               <%
				       }
					   ...
					   <li>
					   <a href="showAllBX.do?size=${requestScope.size }&pagenow=${requestScope.pagenow+1}">
					   <span>»</span>
					   </a>
					   </li>  
					   </c:if>
