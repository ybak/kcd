...
function a3(){
	...
	window.location.href ="selectAllAssessCars.do?size="+s;
}
...
$('li.active').parents('li').addClass('treeview').addClass('active');
	...
	<tbody>
		<tr>
			...
			<th>车型</th>
			<th class="hidden-xs" style="width: 100px">上牌时间</th>
			<th style="width: 200px">公司|姓名</th>
			<th class="hidden-xs" style="width: 80px">里程</th>
			<th class="hidden-xs" style="width: 80px">颜色</th>
			<th class="hidden-xs" style="width: 80px">裸车价</th>
			<th>评估价</th>
			<th class="hidden-xs" style="width: 100px">修改时间</th>
		...
		<tr>
			<td class="text-center hidden-xs"><input type="checkbox" name="delid" value="54103"></td>
			<td class="hidden-xs">${list.carid}</td>
			<td class="hidden-xs">${list.car_days}</td>
			<td class="hidden-xs">${list.mem_name}</td>
			<td class="hidden-xs">${list.car_km}</td>
			<c:if test="${list.color_id eq 1}">
				<td class="hidden-xs">黑</td>
			</c:if>
			<c:if test="${list.color_id eq 2}">
				<td class="hidden-xs">白</td>
			</c:if>
			<c:if test="${list.color_id eq 3}">
				<td class="hidden-xs">灰</td>
			</c:if>
			<c:if test="${list.color_id eq 4}">
				<td class="hidden-xs">红</td>
			</c:if>
			<c:if test="${list.color_id eq 5}">
				<td class="hidden-xs">银</td>
			</c:if>
			<c:if test="${list.color_id eq 6}">
				<td class="hidden-xs">蓝</td>
			</c:if>
			<c:if test="${list.color_id eq 7}">
				<td class="hidden-xs">金</td>
			</c:if>
			<c:if test="${list.color_id eq 8}">
				<td class="hidden-xs">棕</td>
			</c:if>
			<c:if test="${list.color_id eq 9}">
				<td class="hidden-xs">橙</td>
			</c:if>
			<c:if test="${list.color_id eq 10}">
				<td class="hidden-xs">黄</td>
			</c:if>
			<c:if test="${list.color_id eq 11}">
				<td class="hidden-xs">紫</td>
			</c:if>
			<c:if test="${list.color_id eq 12}">
				<td class="hidden-xs">绿</td>
			</c:if>
			<c:if test="${list.color_id eq 13}">
				<td class="hidden-xs">褐</td>
			</c:if>
			<c:if test="${list.color_id eq 14}">
				<td class="hidden-xs">栗</td>
			</c:if>
			<c:if test="${list.color_id eq 15}">
				<td class="hidden-xs">米</td>
			</c:if>
			<c:if test="${list.color_id eq 16}">
				<td class="hidden-xs">银灰</td>
			</c:if>
			<c:if test="${list.color_id eq 17}">
				<td class="hidden-xs">青</td>
			</c:if>
			<c:if test="${list.color_id eq 18}">
				<td class="hidden-xs">香槟</td>
			</c:if>
			<c:if test="${list.color_id eq 19}">
				<td class="hidden-xs">咖啡</td>
			</c:if>
			<c:if test="${list.color_id eq 20}">
				<td class="hidden-xs">天山</td>
			</c:if>
			<c:if test="${list.color_id eq 0}">
				<td class="hidden-xs">其他色</td>
			</c:if>
			<td class="hidden-xs">${list.price_new/10000}万</td>
			<td>${list.price_result/10000}万</td>
			<td class="hidden-xs">${list.dt_edit}</td>
			<!-- 操作 -->
			<td class="text-center">
				<div class="table-button">
					<a href="sAACarsCaoZuo.do?id=${list.id}" class="btn btn-default">
						<i class="fa fa-pencil"></i>
					</a>
					<a href="deleteACars.do?id=${list.id}" id="delid_54103" class="btn btn-default">
						<i class="fa fa-trash"></i>
					</a>
				</div>
			</td>
			<!-- 订单类型 -->
			<c:if test="${list.bc_type eq 0}">
				<td class="hidden-xs">快速评估</td>	
			</c:if>
			<c:if test="${list.bc_type eq 1}">
				<td class="hidden-xs">专业评估</td>	
			</c:if>
			<c:if test="${list.bc_type eq 3}">
				<td class="hidden-xs">点融专用</td>	
			</c:if>
			<!-- 审核状态 -->
			<c:if test="${list.bc_status eq 1}">
				<td><span class="label label-danger">草稿箱</span></td>	
			</c:if>
			<c:if test="${list.bc_status eq 2}">
				<td><span class="label label-danger">正在估值</span></td>	
			</c:if>
			<c:if test="${list.bc_status eq 3}">
				<td><span class="label label-success">估值完成</span></td>	
			</c:if>
			<c:if test="${list.bc_status eq 4}">
				<td><span class="label label-danger">回退</span>	</td>
			</c:if>
			<c:if test="${list.bc_status eq 5}">
				<td><span class="label label-danger">已撤销</span></td>	
			</c:if>
		</tr>
		</c:forEach>
			</tbody>
...
<div class="foot-page">
				...
				<ul class="pagination no-margin">
				       ...
				       <li >
				       <a  href="selectAllAssessCars.do?size=${requestScope.size }&pagenow=${requestScope.pagenow-1}">
				       <span >«</span>
				       </a>
				       </li>
				       </c:if>
					   ...
					   <a id="a<%=j %>" href="selectAllAssessCars.do?size=${requestScope.size }&pagenow=<%=j %>">
					   <%=j %>
					   </a>
					   </li>
					   <%
					   ...
					   %>
	                   <li id="l<%=j %>" >
					   <a id="a<%=j %>" href="selectAllAssessCars.do?&size=${requestScope.size }&pagenow=<%=j %>"><%=j %>					   
					   </a>
					   </li>					   
		               <%
				       }
