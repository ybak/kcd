...
function a3(){
	...
	//alert(s)
	window.location.href ="mdxxlist.do?size="+s;

}
function a2(){
	var khgsname=$("#khgsname").val();
	window.location.href ="mdxxlist.do";

}
function a1(size) {
	//alert(size);
	window.location.href ="mdxxlist.do?size="+size+"&pagenow=3";
}
</script>
...
$('li.active').parents('li').addClass('treeview').addClass('active');
	...
	<tbody>
		<tr>
			<th class="hidden-xs ext-center" style="width:80px">工号</th>
			<th class="hidden-xs ext-center" style="width:80px">门店名称</th>
			<th class="hidden-xs ext-center" style="width:80px">人员名称</th>
			<th class="hidden-xs ext-center" style="width:80px">人员等级</th>
			<th class="hidden-xs ext-center" style="width:80px">身份证号码</th>
			<th class="hidden-xs ext-center" style="width:80px">手机号码</th>
			<th class="hidden-xs ext-center" style="width:80px">添加时间</th>
			<th class="hidden-xs ext-center" style="width:80px">更新时间</th>
			<th style="width: 80px" class="text-center">操作</th>
			<th style="width: 80px" class="text-center">API操作</th>		
			</tr>
				
	<c:forEach items="${requestScope.mdlist }" var="m">			
			<tr>
			<td class="hidden-xs">${m.id }</td>
			<td class="hidden-xs">${m.sname }</td>
			<td class="hidden-xs">${m.pname }</td>
			<td class="hidden-xs">${m.alevel }</td>
			<td class="hidden-xs">${m.pIDcard }</td>
			<td class="hidden-xs">${m.pcall }</td>
			<td class="hidden-xs">${m.addtime }</td>
			<td class="hidden-xs">${m.uptime }</td>
			...
			<td>
			<c:if test="${m.czcode eq 1 }">
			<span class="label label-success">添加</span>
			</c:if>
			<c:if test="${m.czcode != 1 }">
			...
			<%
				...
				%> 
				  <li>
				  <a href="mdxxlist.do?size=${requestScope.size}&pagenow=<%=i+1%>"><%=i+1 %></a>
				  </li> 
				<%				
				}
				%> 
