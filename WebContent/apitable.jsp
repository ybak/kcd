...
function a3(){
	...
	//alert(s);
	window.location.href ="apilist.do?pagesize="+s;
}

function a1(size) {
	alert(size);
	window.location.href ="dsjlist.do?pagesize="+size+"&pagenow=3";
}
</script>	
...
$('li.active').parents('li').addClass('treeview').addClass('active');
	...
	<tbody>
		<tr>
			<th class="hidden-xs ext-center" style="width:80px">商户公司名</th>
			<th class="hidden-xs ext-center" style="width:80px">签约公司名</th>
			<th class="hidden-xs ext-center" style="width:50px">金额</th>
			<th class="hidden-xs ext-center" style="width:80px">备注</th>
			<th class="hidden-xs ext-center" style="width:80px">币种</th>
			<th class="hidden-xs ext-center" style="width:80px">创建时间</th>
			<th class="hidden-xs ext-center" style="width:80px">操作人</th>
			<th class="hidden-xs ext-center" style="width:80px">订单ID</th>			
			<th style="width: 80px" class="text-center">查询状态</th>		
			</tr>
				
			<c:forEach items="${requestScope.apilist }" var="a" >										
			<tr>
			<td class="hidden-xs">${a.gsname }</td>
			<td class="hidden-xs">${a.qygsname }</td>
			<td class="hidden-xs">-${a.je }.00元</td>
			<td class="text-center"><span class="label label-danger">${a.bz }</span></td>
			<td class="hidden-xs">${a.jelx }</td>
			<td class="hidden-xs">${a.time }</td>
			<td class="hidden-xs">${a.czr }</td>
			<td class="hidden-xs">${a.orderid }</td>
...
<c:if test="${requestScope.w ge '1' }">
				<ul class="pagination no-margin">
				       ...
				       <li >
				       <a  href="apilist.do?pagesize=${requestScope.size }&pagenow=${requestScope.pagenow-1}">
				       <span >«</span>
				       </a>
				       </li>
				       </c:if>
					   ...
					   <a id="a<%=j %>" href="apilist.do?pagesize=${requestScope.size }&pagenow=<%=j %>">
					   <%=j %>
					   </a>
					   </li>
					   <%
					   ...
					   %>
	                   <li id="l<%=j %>" >
					   <a id="a<%=j %>" href="apilist.do?pagesize=${requestScope.size }&pagenow=<%=j %>"><%=j %>					   
					   </a>
					   </li>					   
		               <%
				       }
