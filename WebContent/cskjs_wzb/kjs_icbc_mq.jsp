...
<script type="text/javascript">
    ...
    function a2(){
		...
		var querytype=$("#querytype").val();		
		window.location.href ="kjs_mq.do?querytype="+querytype+"&size="+s+"&status="+type_select;
    }
    function a1(){
		...
		<section class="content-header">
			<h1 class="dada">
			视频面签<small>共${requestScope.totalno }个</small>
				...
				<div class="row">
					...
					<div class="col-sm-2 admin-page-top hidden-xs">
					   ...
					   <ul class="pagination no-margin">					     				       
						...
						%>
						<a href="kjs_mq.do?querytype=${requestScope.querytype }&size=${requestScope.size }&pagenow=${requestScope.pagenow-1}&status=${requestScope.status}" class="btn btn-default">«</a>						
						<%				    	
				         }						
						 if(pagenow1>=1&&pagenow1<totalpage1){
						%>
	...
	<tbody>
		...
		<c:forEach items="${requestScope.list }" var="icbc" varStatus="numb">
			...
			<td class="text-center">
				...
				<a href="icbc_sp_sh.do?icbc_type=4&icbc_id=${icbc.icbc_id }&querytype=${requestScope.querytype}&size=${requestScope.size}&status=${requestScope.status}" class="btn btn-default">
				<i class="fa fa-pencil"></i>
				</a>
				</div>
			</td>
			<td><!--当前状态  -->
			<a href="icbc_sp_sh.do?icbc_type=4&icbc_id=${icbc.icbc_id }&querytype=${requestScope.querytype}&size=${requestScope.size}&status=${requestScope.status}">
			<span id="mq${icbc.id }"  class="label label-warning">${icbc.statusname }</span></a>
			</td>	
			</tr>
			</c:forEach>
			</tbody>
  ...
  <script type="text/javascript">
	...
	<div class="foot-page">
			...
			<ul class="pagination no-margin">
				       ...
				       <li >
				       <a  href="kjs_mq.do?querytype=${requestScope.querytype }&size=${requestScope.size}&pagenow=${requestScope.pagenow-1}&status=${requestScope.status}">
				       <span >«</span>
				       </a>
				       </li>
				       </c:if>
					   ...
					   <a id="a<%=j %>" href="kjs_mq.do?querytype=${requestScope.querytype }&size=${requestScope.size }&pagenow=<%=j %>&status=${requestScope.status}">
					   <%=j %>
					   </a>
					   </li>
					   <%
					   ...
					   %>
	                   <li id="l<%=j %>" >
					   <a id="a<%=j %>" href="kjs_mq.do?querytype=${requestScope.querytype }&size=${requestScope.size }&pagenow=<%=j %>&status=${requestScope.status}"><%=j %>					   
					   </a>
					   </li>					   
		               <%
				       }
