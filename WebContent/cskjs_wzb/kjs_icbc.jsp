...
<script type="text/javascript">
    function a3(){
		...
		window.location.href ="newicbc.do?querytype="+querytype+"&size="+s+"&status="+status;
    }
    function a2(){
		...
		var querytype=$("#querytype").val();		
		window.location.href ="newicbc.do?querytype="+querytype+"&size="+s+"&status="+type_select;
    }
    function a1(){
		...
		<section class="content-header">
			...
			<option value="4" ${requestScope.status==4?"selected='selected'":''}>回退</option>
			</select>						
			</h1>			
		</section>
		<!-- Main content -->
				...
				<div class="row">
					...
					<div class="col-sm-2 admin-page-top hidden-xs">
					   ...
					   <ul class="pagination no-margin">					     				       
						...
						%>
						<a href="newicbc.do?querytype=${requestScope.querytype }&size=${requestScope.size }&pagenow=${requestScope.pagenow-1}&status=${requestScope.status}" class="btn btn-default">«</a>						
						<%				    	
				         }						
						 if(pagenow1>=1&&pagenow1<totalpage1){
						%>
	...
	<tbody>
		...
		<c:forEach items="${requestScope.list }" var="icbc" varStatus="numb">
			...
			<td class="text-center">
				...
				<span id="cn${icbc.id }"  class="label label-danger"></span>
				</c:if>
			...
			<td class="text-center hidden-xs">
					...
					<a data-toggle="modal" style="" data-target="#modal" href="icbcyl.do?type=1&name=${icbc.c_name }&cardid=${icbc.c_cardno}" class="btn btn-default">
  ...
  <script type="text/javascript">
	...
	<div class="foot-page">
			...
			<ul class="pagination no-margin">
				       ...
				       <li >
				       <a  href="newicbc.do?querytype=${requestScope.querytype }&size=${requestScope.size}&pagenow=${requestScope.pagenow-1}&status=${requestScope.status}">
				       <span >«</span>
				       </a>
				       </li>
				       </c:if>
					   ...
					   <a id="a<%=j %>" href="newicbc.do?querytype=${requestScope.querytype }&size=${requestScope.size }&pagenow=<%=j %>&status=${requestScope.status}">
					   <%=j %>
					   </a>
					   </li>
					   <%
					   ...
					   %>
	                   <li id="l<%=j %>" >
					   <a id="a<%=j %>" href="newicbc.do?querytype=${requestScope.querytype }&size=${requestScope.size }&pagenow=<%=j %>&status=${requestScope.status}"><%=j %>					   
					   </a>
					   </li>					   
		               <%
				       }
