...
$(document).ready(function(){
    ...
    function a3(){
		...
		var icbcname=$("#icbcname").val();
		var querytype=$("#querytype").val();		
		window.location.href ="search_icbc.do?"
			    +"icbcname="+icbcname
				+"&card_status="+card_status
				+"&book_status="+book_status
				+"&gems_fs_id="+gems_fs_id+"&bc_status="+status+"&time="+time
				+"&querytype="+querytype+"&size="+s+"&status="+status;
    }
    function a2(){
		var time=$("#time").val();
		var gems_fs_id=$("#gems_fs_id").val();
		...
		window.location.href ="search_icbc.do?"
			...
			+"&card_status="+card_status
			+"&book_status="+book_status
			+"&gems_fs_id="+gems_fs_id+"&bc_status="+type_select+"&time="+time
			+"&querytype="+querytype+"&size="+s+"&status="+type_select;
    }
    function a1(){
		var time=$("#time").val();
		var gems_fs_id=$("#gems_fs_id").val();
		...
		<section class="content-header">
			<h1 class="dada">
			工行贷<small>共${requestScope.totalno }个</small>
			<select class="selector" id="type_select1" onchange="a1()" style="float: right;">	
			<option value="0" ${requestScope.querytype==0?"selected='selected'":''}">全部渠道</option>
			<option value="1" ${requestScope.querytype==1?"selected='selected'":''}">APP</option>
			<option value="2" ${requestScope.querytype==2?"selected='selected'":''}">API</option>
			</select>
			<select class="selector" id="type_select" onchange="a2()" style="float: right;margin-right: 20px;">	
			<option value="" ${requestScope.status==1?"selected='selected'":''}>全部订单</option>
			...
			<div class="admin-tools">
				<div class="row">
					<div class="col-sm-10 admin-button">
							...
							新增</a>																															
							<a href="toExcel.do?time=${requestScope.time }&querytype=${requestScope.querytype}&bc_status=${requestScope.bc_status}&gems_fs_id=${requestScope.gems_fs_id}&book_status=${requestScope.book_status }&card_status=${requestScope.card_status }&icbcname=${requestScope.icbcname }&type=search" style="background-color: #ff8700;" class="btn btn-default">
							导出
							</a>
							</div>
					</div>
					<div class="col-sm-2 admin-page-top hidden-xs">
					   ...
					   <ul class="pagination no-margin">					     				       
						...
						%>
						<a href="search_icbc.do?time=${requestScope.time }&bc_status=${requestScope.bc_status }&gems_fs_id=${requestScope.gems_fs_id}&book_status=${requestScope.book_status }&card_status=${requestScope.card_status }&icbcname=${requestScope.icbcname }&querytype=${requestScope.querytype }&size=${requestScope.size }&pagenow=${requestScope.pagenow-1}&status=${requestScope.status}" class="btn btn-default">«</a>						
						<%				    	
				         }						
						 if(pagenow1>=1&&pagenow1<totalpage1){
						%>
	...
	<tbody>
		...
		<c:forEach items="${requestScope.list }" var="icbc" varStatus="numb">
			...
			<div class="autocut" title="${icbc.gems_code }">
			${fn:substring(icbc.gems_code,9,16)}
			</div>
			</td>
			<td class="text-center">
            <c:if test="${icbc.query_type eq '2'}">
            API来源:${icbc.gname}-${icbc.pname}
            </c:if>
            <c:if test="${icbc.query_type ne '2'}">
            APP来源:${icbc.gname}-${icbc.pname}
            </c:if>
            </td>
			<td class="hidden-xs text-center">
			...
			<td class="text-center">
				...
				<c:if test="${icbc.adminop_tag eq '1' }">
				<input type="hidden" id="aid" name="aid" value="${icbc.id }" />
				<span id="cn${icbc.id }" name="cn${icbc.id }"  class="label label-danger">
				</span>
				</c:if>
				<a href="newicbcsh.do?mid_edit=${sessionScope.id }&icbc_type=1&id=${icbc.id }&querytype=${requestScope.querytype}&size=${requestScope.size}&status=${requestScope.status}" class="btn btn-default">
				<i class="fa fa-pencil"></i>
				</a>
				<c:if test="${sessionScope.id==2538}">
				<a href="javascript:del_icbc('${icbc.id }')" class="btn btn-default">
			...
			<td><!--当前状态  -->
			<c:if test="${icbc.bc_status==3}">
            <a href="newicbcsh.do?mid_edit=${sessionScope.id }&icbc_type=1&id=${icbc.id }&querytype=${requestScope.querytype}&size=${requestScope.size}&status=${requestScope.status}" >
            <span class="label label-success" >${icbc.statusname }</span></a> 			
			</c:if>
			<c:if test="${icbc.bc_status!=3}">
			<c:if test="${icbc.tr_status==2}">
			<a href="newicbcsh.do?mid_edit=${sessionScope.id }&icbc_type=1&id=${icbc.id }&querytype=${requestScope.querytype}&size=${requestScope.size}&status=${requestScope.status}" >
			<span class="label label-success" >${icbc.statusname }</span></a>
			</c:if >
			<c:if test="${icbc.tr_status!=2}">
			...
			<td><!--当前状态  -->
			<a href="icbc_cars_sh.do?icbc_type=3&icbc_id=${icbc.id }&querytype=${requestScope.querytype}&size=${requestScope.size}&status=${requestScope.status}">
			<span id="pg${icbc.id }"  class="label label-warning" >${icbc.statusname5 }</span></a>
			</td>	
			<td><!--当前状态  -->
			<a href="icbc_sp_sh.do?icbc_type=4&icbc_id=${icbc.id }&querytype=${requestScope.querytype}&size=${requestScope.size}&status=${requestScope.status}">
			<span id="mq${icbc.id }"  class="label label-warning">${icbc.statusname4 }</span></a>
			</td>	
			<td><!--当前状态  -->
			<a href="kk_sh.do?icbc_type=5&icbc_id=${icbc.id }&querytype=${requestScope.querytype}&size=${requestScope.size}&status=${requestScope.status}">
			<span id="kk${icbc.id }" class="label label-warning">${icbc.statusname3 }</span></a>
			</td>	
			<td><!--当前状态  -->
			<a href="dk_sh.do?icbc_type=6&icbc_id=${icbc.id }&querytype=${requestScope.querytype}&size=${requestScope.size}&status=${requestScope.status}">			
			<span id="dk${icbc.id }"  class="label label-warning">${icbc.statusname2 }</span></a>
			</td>	
			<td>
			<select id="checkstatus_${icbc.id }" onchange="fkup('${icbc.id }')" class="form-control">
					<option value="0" >待选择</option>
					<option value="1" ${icbc.fk_status==1?"selected='selected'":''}>未垫资/放款</option>
					<option value="2" ${icbc.fk_status==2?"selected='selected'":''}>已垫资</option>
					<option value="3" ${icbc.fk_status==3?"selected='selected'":''}>已放款</option>				
			</select>
            </td>
            <td>
			<a href="javascript:khhc('${icbc.id }');">
...
function fkup(id){
	...
	<div class="foot-page">
			...
			<ul class="pagination no-margin">
				       ...
				       <li >
				       <a  href="search_icbc.do?time=${requestScope.time }&bc_status=${requestScope.bc_status }&gems_fs_id=${requestScope.gems_fs_id}&book_status=${requestScope.book_status }&card_status=${requestScope.card_status }&icbcname=${requestScope.icbcname }&querytype=${requestScope.querytype }&size=${requestScope.size }&pagenow=${requestScope.pagenow-1}&status=${requestScope.status}">
				       <span >«</span>
				       </a>
				       </li>
				       </c:if>
					   ...
					   <a id="a<%=j %>" href="search_icbc.do?time=${requestScope.time }&bc_status=${requestScope.bc_status }&gems_fs_id=${requestScope.gems_fs_id}&book_status=${requestScope.book_status }&card_status=${requestScope.card_status }&icbcname=${requestScope.icbcname }&querytype=${requestScope.querytype }&size=${requestScope.size }&pagenow=<%=j %>&status=${requestScope.status}">
					   <%=j %>
					   </a>
					   </li>
					   <%
					   ...
					   %>
	                   <li id="l<%=j %>" >
					   <a id="a<%=j %>" href="search_icbc.do?time=${requestScope.time }&bc_status=${requestScope.bc_status }&gems_fs_id=${requestScope.gems_fs_id}&book_status=${requestScope.book_status }&card_status=${requestScope.card_status }&icbcname=${requestScope.icbcname }&querytype=${requestScope.querytype }&size=${requestScope.size }&pagenow=<%=j %>&status=${requestScope.status}"><%=j %>					   
					   </a>
					   </li>					   
		               <%
				       }
