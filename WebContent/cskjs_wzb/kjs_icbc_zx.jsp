...
<script type="text/javascript">
    ...
    function a2(){
		...
		var querytype=$("#querytype").val();		
		window.location.href ="kjs_zx.do?querytype="+querytype+"&size="+s+"&status="+type_select;
    }
    function a1(){
		...
		<section class="content-header">
			<h1 class="dada">
			征信<small>共${requestScope.totalno }个</small>
			...
			<option value="7" ${requestScope.status==7?"selected='selected'":''}>征信不通过(用户提交通融)</option>
				...
				<div class="row">
					...
					<div class="col-sm-2 admin-page-top hidden-xs">
					   ...
					   <ul class="pagination no-margin">					     				       
						...
						%>
						<a href="kjs_zx.do?querytype=${requestScope.querytype }&size=${requestScope.size }&pagenow=${requestScope.pagenow-1}&status=${requestScope.status}" class="btn btn-default">«</a>						
						<%				    	
				         }						
						 if(pagenow1>=1&&pagenow1<totalpage1){
						%>
	...
	<tbody>
		...
		<c:forEach items="${requestScope.list }" var="icbc" varStatus="numb">
			...
			<td class="hidden-xs text-center">${icbc.c_cardno}</td>
			<td class="hidden-xs text-center">
			<div class="autocut" title="${icbc.zx_result}">
			${fn:substring(icbc.zx_result,0,20)}...
			</div>
			</td>
			<td class="text-center">
				<!--  编辑-->
  ...
  <script type="text/javascript">
	...
	<div class="foot-page">
			...
			<ul class="pagination no-margin">
				       ...
				       <li >
				       <a  href="kjs_zx.do?querytype=${requestScope.querytype }&size=${requestScope.size}&pagenow=${requestScope.pagenow-1}&status=${requestScope.status}">
				       <span >«</span>
				       </a>
				       </li>
				       </c:if>
					   ...
					   <a id="a<%=j %>" href="kjs_zx.do?querytype=${requestScope.querytype }&size=${requestScope.size }&pagenow=<%=j %>&status=${requestScope.status}">
					   <%=j %>
					   </a>
					   </li>
					   <%
					   ...
					   %>
	                   <li id="l<%=j %>" >
					   <a id="a<%=j %>" href="kjs_zx.do?querytype=${requestScope.querytype }&size=${requestScope.size }&pagenow=<%=j %>&status=${requestScope.status}"><%=j %>					   
					   </a>
					   </li>					   
		               <%
				       }
