...
function a3(){
	...
	//alert(s);
	window.location.href ="hmdcx.do?pagesize="+s;
}
</script>	
...
$('li.active').parents('li').addClass('treeview').addClass('active');
	...
	<tbody>
			...
			<th class="hidden-xs ext-center" style="width:80px">完成时间</th>
			...
			<td class="hidden-xs">-3.00元</td>
			<td class="hidden-xs">${t.name }</td>
			<td class="hidden-xs">${t.idcard }</td>
			<td class="hidden-xs">${t.phonenum }</td>
		    <td class="hidden-xs">
		    ${fn:substring(t.addtime, 0,19)}
		    </td>
			<td class="hidden-xs">
			${fn:substring(t.uptime, 0,19)}
			</td>	
			<td class="text-center">
			<span class="label label-danger">${t.lx }</span>
			</td>
			<td class="text-center"><span class="label label-success">查询完成</span>
				...
				<ul class="pagination no-margin">
				       ...
				       <li >
				       <a  href="hmdcx.do?pagesize=${requestScope.size }&pagenow=${requestScope.pagenow-1}">
				       <span >«</span>
				       </a>
				       </li>
				       </c:if>
					   ...
					   <a id="a<%=j %>" href="hmdcx.do?pagesize=${requestScope.size }&pagenow=<%=j %>">
					   <%=j %>
					   </a>
					   </li>
					   <%
					   ...
					   %>
	                   <li id="l<%=j %>" >
					   <a id="a<%=j %>" href="hmdcx.do?pagesize=${requestScope.size }&pagenow=<%=j %>"><%=j %>					   
					   </a>
					   </li>					   
		               <%
				       }
