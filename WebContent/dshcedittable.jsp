...
function a3(){
	...
	window.location.href ="zxbybit.do?size="+s;

}
function a2(){
	var khgsname=$("#khgsname").val();
	window.location.href ="zxbybit.do";

}
function a1(size) {
	alert(size);
	window.location.href ="zxbybit.do?size="+size+"&pagenow=3";
}
</script>
...
$('li.active').parents('li').addClass('treeview').addClass('active');
		...
		<section class="content-header">
			<h1>
			征信材料待审核订单<small>共${requestScope.totalCount }个</small>
			</h1>
		</section>
		...
		<section class="content">
			...
			<div class="admin-tools">
				...
				<div class="col-sm-7 admin-button">
					...
					<!-- 增加删除按钮 --> 
					    <div class="btn-group">								
							<a href="" class="btn btn-default"><i class="fa fa-edit"></i> 
							导出</a>
							<a href="" class="btn btn-default" data-toggle="control-sidebar"><i class="fa fa-search"></i>
							 查询</a>	
					...
					<div class="col-sm-5 admin-page-top hidden-xs">
					            <select id="khgsname" onchange="" class="form-control">
	...
	<tbody>
		    <tr>
			<th class="hidden-xs ext-center" style="width:80px">来源公司-操作人员</th>
			<th class="hidden-xs ext-center" style="width:80px">身份信息</th>			
			<th class="hidden-xs ext-center" style="width:120px">建单时间</th>
			<th class="hidden-xs ext-center" style="width:120px">最后更新时间</th>
			<th style="width: 120px" class="text-center">操作状态</th>
			...
			<tr>
			<td class="hidden-xs">${c.gsname }-${c.ryname }</td>
			...
			<%
				...
				%> 
				  <li>
				  <c:choose>
				  <c:when test="${!empty requestScope.bit}">
				  <a href="zxbybit.do?size=${requestScope.size}&pagenow=<%=i+1%>&bit=${requestScope.bit}"><%=i+1 %></a>
				  </c:when>
				  <c:otherwise>
				  <a href="zxbybit.do?size=${requestScope.size}&pagenow=<%=i+1%>"><%=i+1 %></a>
				  </c:otherwise>
				  </c:choose>				
				  </li> 
				<%
...
<div class="daterangepicker dropdown-menu show-calendar opensright" style="top: 156px; left: 1394px; right: auto;">
...
<tr>
<th></th>
<th colspan="5" class="month">八月 2017</th>
...
<td class="off disabled" data-title="r2c2">15</td>
<td class="off disabled" data-title="r2c3">16</td>
<td class="off disabled" data-title="r2c4">17</td>
<td class="off disabled" data-title="r2c5">18</td>
