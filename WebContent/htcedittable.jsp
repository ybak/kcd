...
function a3(){
	...
	window.location.href ="htlist.do?size="+s;

}
function a2(){
	var khgsname=$("#khgsname").val();
	window.location.href ="htlist.do";

}
function a1(size) {
	alert(size);
	window.location.href ="htlist.do?size="+size+"&pagenow=3";
}
</script>
...
$('li.active').parents('li').addClass('treeview').addClass('active');
	...
	<tbody>
			...
			<c:choose>
			<c:when test="${requestScope.khgsname == null }">
							<%
				...
				%> 
				  <li>
				  <a href="htlist.do?size=${requestScope.size}&pagenow=<%=i+1%>"><%=i+1 %></a>
				  </li> 
				<%
