...
<script type="text/javascript">
    function a3(){
		...
		window.location.href ="TKJSselectAllCar.do?size="+s+"&status="+status;
...
<table class="table table-bordered table-hover">
	<tbody>
		<tr>
			...
			<tr>
				...
				<td class="text-center"><%-- ${cert.dt_edit} --%>${fn:substring(cert.dt_edit,0,16)}</td>
				<td class="text-center">${cert.c_name}</td>
				<td class="text-center hidden-xs">
					<!--  签约--> 
					<c:choose>			
						<c:when test="${requestScope.status == 3}">
							<a data-toggle="modal" style="" data-target="#modal" class="btn btn-success" href="goToOneCar.do?idcard=${cert.c_cardno}&cname=${cert.c_name}"><i class="fa fa-credit-card"></i></a>
						</c:when>
						<c:otherwise>
							<a data-toggle="modal" style="" data-target="#modal" class="btn btn-default" href="goToOneCar.do?idcard=${cert.c_cardno}&cname=${cert.c_name}"><i class="fa fa-credit-card"></i></a>
						</c:otherwise>
					</c:choose>
					<!--  分期代收--> 
					<a data-toggle="modal" style="" data-target="#modal" class="btn btn-warning" href="goToTwoCar.do?type=2&mgcertId=${cert.id}&khName=${cert.c_name}&khCardId=${cert.c_cardno}&status=${requestScope.status}"><i class="fa fa-soccer-ball-o"></i></a> 
					<!--  代收-->
					<a data-toggle="modal" style="" data-target="#modal" class="btn btn-success" href="goToTwoCar.do?type=3&mgcertId=${cert.id}&khName=${cert.c_name}&khCardId=${cert.c_cardno}&status=${requestScope.status}"><i class="fa fa-sign-in"></i></a> 
					<!-- 代付 -->
					<a data-toggle="modal" style="" data-target="#modal" class="btn btn-danger" href="goToTwoCar.do?type=4&mgcertId=${cert.id}&khName=${cert.c_name}&khCardId=${cert.c_cardno}&status=${requestScope.status}"><i class="fa fa-sign-out"></i></a>
				</td>
				<td class="text-center">
					<div class="table-button">
					<a href="CaoZuoCar.do?type=8&khName=${cert.c_name}&khCardId=${cert.c_cardno}&amid=${cert.id}&fsname=${cert.name}&gemsname=${cert.namee}" class="btn btn-default"><i class="fa fa-pencil"></i></a>
	                </div>
		   	    </td>
		    </tr>
            </c:forEach>
	...
	<div class="foot-page">
			...
			<ul class="pagination no-margin">
				       ...
				       <li >
				       <a  href="TKJSselectAllCar.do?size=${requestScope.size}&pagenow=${requestScope.pagenow-1}&status=${requestScope.status}">
				       <span >«</span>
				       </a>
				       </li>
				       </c:if>
					   ...
					   <a id="a<%=j %>" href="TKJSselectAllCar.do?size=${requestScope.size }&pagenow=<%=j %>&status=${requestScope.status}">
					   <%=j %>
					   </a>
					   </li>
					   <%
					   ...
					   %>
	                   <li id="l<%=j %>" >
					   <a id="a<%=j %>" href="TKJSselectAllCar.do?size=${requestScope.size }&pagenow=<%=j %>&status=${requestScope.status}"><%=j %>					   
					   </a>
					   </li>					   
		               <%
				       }
