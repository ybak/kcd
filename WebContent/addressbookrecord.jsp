...
function a3(){
	...
	//alert(s);
	window.location.href ="txllist.do?pagesize="+s;
}
</script>	
...
$('li.active').parents('li').addClass('treeview').addClass('active');
	...
	<tbody>
			...
			<th class="hidden-xs ext-center" style="width:80px">创单时间</th>
			<th class="hidden-xs ext-center" style="width:80px">查询报告时间</th>
			<th class="hidden-xs ext-center" style="width:80px">查询原始数据时间</th>
			<th style="width: 120px" class="text-center">操作类型</th>
			<th style="width: 80px" class="text-center">查询状态</th>		
			</tr>
				
			<c:forEach items="${requestScope.clist }" var="t">									
			<tr>
			<td class="hidden-xs">${t.gsname }-${t.ryname }</td>
			<td class="hidden-xs">-10.00元</td>
			<td class="hidden-xs">${t.name }</td>
			<td class="hidden-xs">${t.idcard }</td>
			<td class="hidden-xs">${t.phonenum }</td>
		    <td class="hidden-xs">${t.stime }</td>
			<td class="hidden-xs">${t.etime1 }</td>
		    <td class="hidden-xs">${t.etime2 }</td>		
			<td class="text-center">
			<span class="label label-danger">${t.cxlx }</span>
			</td>
			<td class="text-center"><span class="label label-success">查询完成</span>
			</td>
		    </tr>
		    </c:forEach>
				...
				<ul class="pagination no-margin">
				       ...
				       <li >
				       <a  href="txllist.do?pagesize=${requestScope.size }&pagenow=${requestScope.pagenow-1}">
				       <span >«</span>
				       </a>
				       </li>
				       </c:if>
					   ...
					   <a id="a<%=j %>" href="txllist.do?pagesize=${requestScope.size }&pagenow=<%=j %>">
					   <%=j %>
					   </a>
					   </li>
					   <%
					   ...
					   %>
	                   <li id="l<%=j %>" >
					   <a id="a<%=j %>" href="txllist.do?pagesize=${requestScope.size }&pagenow=<%=j %>"><%=j %>					   
					   </a>
					   </li>					   
		               <%
				       }
