...
function a2(){
	...
	//alert(s)
	window.location.href ="limitfinance.do?size="+s;
...
<!-- 	<body class="skin-green sidebar-mini fixed"> -->
	...
	<!-- 		<th style="width: 3%" class="text-center hidden-xs"><input type="checkbox" class="check_all"></th> -->
			...
			<th class="hidden-xs" style="width: 130px">商户公司</th>
			<th class="hidden-xs" style="width: 130px">签约公司</th>
			...
			<td>${fn.fid }</td>
			<td class="hidden-xs">${fn.shgs }</td>
			<td class="hidden-xs">${fn.qygsm }</td>
			<td class="hidden-xs">${fn.je }</td>
			<td class="hidden-xs">${fn.rmk }</td>
			<td class="hidden-xs">${fn.bz }</td>
		    <td class="hidden-xs">${fn.lx }</td>
			<td class="hidden-xs">		
			${fn.addtime}
			</td>
			<td><span class="label label-success">充值完成</span></td>
			<td class="hidden-xs">${fn.czr }</td>	
			<td class="text-center">
				<div class="table-button">
					<a href="" class="btn btn-default">
					<i class="fa fa-pencil"></i>
					</a>
					<a href="" id="delid_37581" class="btn btn-default">
			...
			<td>1</td>
			<td class="hidden-xs">上海A公司</td>
			<td>上海B公司</td>
			<td class="hidden-xs">5000</td>
			<td class="hidden-xs">后台充值</td>
			<td class="hidden-xs">现金</td>
			<td>正常充值</td>
			<td class="hidden-xs">2017-08-30 09:30:55</td>
    ...
    $(this).removeClass("active");
			...
			<c:if test="${requestScope.w ge '1' }">
				<ul class="pagination no-margin">
				       ...
				       <li >
				       <a  href="limitfinance.do?size=${requestScope.size }&pagenow=${requestScope.pagenow-1}">
				       <span >«</span>
				       </a>
				       </li>
				       </c:if>
					   ...
					   <a id="a<%=j %>" href="limitfinance.do?size=${requestScope.size }&pagenow=<%=j %>">
					   <%=j %>
					   </a>
					   </li>
					   <%
					   ...
					   %>
	                   <li id="l<%=j %>" >
					   <a id="a<%=j %>" href="limitfinance.do?size=${requestScope.size }&pagenow=<%=j %>"><%=j %>					   
					   </a>
					   </li>					   
		               <%
				       }
				...
				<div class="page-num">共${requestScope.count }个 分${requestScope.w }页显示</div>
			</div>
					</section><!-- /.content -->
		</body>
