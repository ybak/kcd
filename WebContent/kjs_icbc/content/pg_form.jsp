...
<input type="hidden" value="3" id="colortype" name="colortype">
	<ul id="myTab" class="nav nav-tabs">
			...
			-->
			<li class="active">
			<a id="icbc3" style="background-color: rgb(255, 135, 0); color: rgb(255, 255, 255);" href="#">
			评估</a></li>
			<!-- <li>
			<a id="icbc4" style="background-color: #b1bacb;color: #ffffff;" href="icbc_sp_sh.do?icbc_type=4&amp;icbc_id=485&amp;querytype=&amp;size=15&amp;status=">
			视频</a></li>
			<li>
...
<script type="text/javascript">
  function getprice1(id){
   	$.ajax({ 
   		url:"${pageContext.request.contextPath}/getNewCarPrice.do",
   	    type:"POST",
   	    dataType:"json",
   	    data:{id:id},
        success:function(msg){
...
<a href="${pageContext.request.contextPath}/downloadFiles_all.do?type=pg&id=${requestScope.id }" class="btn btn-success" style="margin-left:100px">一键下载所有图片</a>
			...
			<div  style="color:#ff8700;" >车辆资料</div>		
				...
				<div class="col-sm-12" style="width: 85%;margin-left:100px; margin-top: 50px;">
					...
					<div class="col-sm-2" style="margin:10px 10px;text-align: center;">
							...
							<a style="padding: 0px 5px;" class="btn btn-warning" id="" href="${pageContext.request.contextPath}/downloadFile.do?fileUrl=http://a.kcway.net/${imgs.bcimg}&fileName=<%=a.nextInt(10000) %>" >下载</a>							
...
function icbc_carsup() {
	...
	color_id=document.getElementById("color_id").value;
 	...
 	$.ajax({
		   type:"post",   
		   url:"${pageContext.request.contextPath}/erp/icbc_cars.do",
		   data:{
			   id : id,
			   adminid : adminid,
			   bc_status : bc_status,
...
<c:forEach items="${requestScope.bList}" var="re">${fn:substring(re.dt_add,0,19)}:状态：${re.statusremark },留言：${re.remark }
		...
		<div class="footer-wrap">
			<div class="box-footer" >
				<button type="button" class="btn btn-default" onclick="location.href='${pageContext.request.contextPath}/erp/index_.do?type=wlghd&dn=pg&qn=list&status=${requestScope.status}'">取消返回</button>
