  ...
  elem: '#date2' //指定元素
			...
			<div class="admin-tools">
				<div class="row">
					...
					<a href="${pageContext.request.contextPath }/erp/index.do?type=dqywgl8&dn=assess_gems&qn=form&cn=1" class="btn btn-default"><i class="fa fa-edit"></i> 新增</a>
					...
					<div class="col-sm-2 admin-page-top hidden-xs">
							<select id="page_limit_select" onchange="window.location.href='${pageContext.request.contextPath }/erp/lengding_list.do?type=wdrw&cn=${requestScope.cn}&dn=lendingResultHzj&qn=list&assess_cp=${requestScope.assess_cp}&selectMsg=${requestScope.selectMsg}&pagenow=${requestScope.pagenow}&pagesize='+this.value+'&fsid=1'" class="form-control">
		...
		<div class="btn-group">
						...
						%>
						<a href="${pageContext.request.contextPath }/erp/lengding_list.do?type=wdrw&cn=${requestScope.cn}&dn=lendingResultHzj&qn=list&assess_cp=${requestScope.assess_cp}&selectMsg=${requestScope.selectMsg}&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}&fsid=${requestScope.fsid }" class="btn btn-default">«</a>						
						<%				    	
				         }						
						 if(pagenow1>=1&&pagenow1<totalpage1){
						%>
	...
	<tbody>
		...
		<tr>
			...
			<td class="text-center">
				...
				<a href="${pageContext.request.contextPath }/erp/index.do?type=dqywgl8&dn=assess_gems&qn=form&id=${pd.id }&cn=4" class="btn btn-default">
				<i class="fa fa-search-plus"></i>
				</a>
				<c:if  test="${fn:contains(sessionScope.pd.purview_map,'ryglupdate')==true}">
					<a href="${pageContext.request.contextPath }/erp/index.do?type=dqywgl8&dn=assess_gems&qn=form&id=${pd.id }&cn=3" class="btn btn-default">
			 ...
			 --%>
				...
				function ajaxdel(id){
					if(window.confirm('你确定要删除吗？')){
				          //alert("确定");
				          window.location.href="${pageContext.request.contextPath }/erp/user_del.do?type=dqywgl8&dn=assess_gems&qn=list&id="+id;
					}else{
			...
			<c:if test="${requestScope.totalpage ge '1' }">
				<ul class="pagination no-margin">
				       <c:if test="${requestScope.pagenow ne '1' }">
				        <li><a href="${pageContext.request.contextPath }/erp/lengding_list.do?type=dqywgl8&dn=lendingResultHzj&cn=${requestScope.cn}&qn=list&assess_cp=${requestScope.assess_cp}&selectMsg=${requestScope.selectMsg}&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}" aria-label="Next"><span aria-hidden="true">«</span></a></li>
				       </c:if>
					   <%
					   ...
					   <a id="a<%=j %>" href="${pageContext.request.contextPath }/erp/lengding_list.do?type=dqywgl8&dn=lendingResultHzj&cn=${requestScope.cn }&qn=list&assess_cp=${requestScope.assess_cp}&selectMsg=${requestScope.selectMsg}&pagesize=${requestScope.pagesize}&pagenow=<%=j %>">
					   <%=j %>
					   </a>
					   </li>
					   <%
