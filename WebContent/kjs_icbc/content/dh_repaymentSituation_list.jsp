...
<script type="text/javascript">
    function a3(){
		...
		window.location.href ="${pageContext.request.contextPath}/repaymentController/select.do?type=khhkqk&dn=dh_repaymentSituation&qn=list&pagesize="+s+"&pagenow=1";
    }
    </script>
		...
		<section class="content">
			<div class="admin-tools">
				<div class="row">
					<div class="col-sm-10">
						<div class="btn-group">		
							<form class="form-horizontal" action="${pageContext.request.contextPath }/repaymentController/select.do?type=khhkqk&dn=dh_repaymentSituation&qn=list&pagesize=9&pagenow=1" method="post">
					...
					<div class="col-sm-2 admin-page-top hidden-xs">
					   ...
					   <ul class="pagination no-margin">					     				       
						...
						<li>
							...
							%>
							<a href="${pageContext.request.contextPath }/repaymentController/select.do?type=khhkqk&dn=dh_repaymentSituation&qn=list&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}" class="btn btn-default">«</a>						
							<%				    	
					         }						
							 if(pagenow1>=1&&pagenow1<totalpage1){
							%>
		...
		%>
			...
			%>
					<tr>
						...
						<td class="text-center">
							<!-- 还款情况详情 -->
							<a href="${pageContext.request.contextPath }/repaymentController/selectBorrow.do?type=khhkqk&dn=dh_repaymentSituation&qn=form&icbc_id=${pd.id}">
			...
			<c:if test="${requestScope.totalpage ge '1' }">
				<ul class="pagination no-margin">
				       <c:if test="${requestScope.pagenow ne '1' }">
				        <li><a href="${pageContext.request.contextPath }/repaymentController/select.do?type=khhkqk&dn=dh_repaymentSituation&qn=list&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}" aria-label="Next"><span aria-hidden="true">«</span></a></li>
				       </c:if>
					   <%
					   ...
					   <a id="a<%=j %>" href="${pageContext.request.contextPath }/repaymentController/select.do?type=khhkqk&dn=dh_repaymentSituation&qn=list&pagesize=${requestScope.pagesize}&pagenow=<%=j %>">
					   <%=j %>
					   </a>
					   </li>
					   <%
