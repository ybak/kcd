...
<script type="text/javascript">
    function a3(){
		...
		window.location.href ="${pageContext.request.contextPath}/uploadExcelController/select.do?type=wlghd&dn=dh_repaymentEntry&qn=list&pagesize="+s+"&pagenow=1";
    }
		</script>	
		...
		<input type="hidden" id="size" name="size" value="${reqeustScope.pagesize}">
			<div class="admin-tools">
				<div class="row">
					<div class="col-sm-10">
						<div class="btn-group">		
							<form style="float: left;" class="form-horizontal" id="ajaxForm" action="${pageContext.request.contextPath }/uploadExcelController/select.do?type=wdrw&dn=dh_repaymentEntry&qn=list&cn=w1&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}" enctype="multipart/form-data" method="post">
					...
					<div class="col-sm-2 admin-page-top hidden-xs">
					   ...
					   <ul class="pagination no-margin">					     				       
						...
						<li>
							...
							%>
							<a href="${pageContext.request.contextPath }/uploadExcelController/select.do?type=wlghd&dn=dh_repaymentEntry&qn=list&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}" class="btn btn-default">«</a>						
							<%				    	
					         }						
							 if(pagenow1>=1&&pagenow1<totalpage1){
							%>
		...
		<div id="main_list" class="admin-content box">
			...
			<table class="table table-bordered table-hover">
				<tbody id="titleBox">	
					...
					<c:forEach var="l1" items="${newpdList }">
					<tr>
						<td class="text-center hidden-xs"><input name="delid" value="" type="checkbox"></td>
						<td class="text-center hidden-xs">${l1.uuid }</td>
			...
			<c:if test="${requestScope.totalpage ge '1' }">
				<ul class="pagination no-margin">
				       <c:if test="${requestScope.pagenow ne '1' }">
				        <li><a href="${pageContext.request.contextPath }/uploadExcelController/select.do?type=wlghd&dn=dh_repaymentEntry&qn=list&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}" aria-label="Next"><span aria-hidden="true">«</span></a></li>
				       </c:if>
					   <%
					   ...
					   <a id="a<%=j %>" href="${pageContext.request.contextPath }/uploadExcelController/select.do?type=wlghd&dn=dh_repaymentEntry&qn=list&pagesize=${requestScope.pagesize}&pagenow=<%=j %>">
					   <%=j %>
					   </a>
					   </li>
					   <%
		    ...
		    function ajax_upload(feid, callback) { //具体的上传图片方法
		        ...
		        if (image_check(feid)) { //自己添加的文件后缀名的验证
		            $.ajaxFileUpload({
		                ...
		                url:"${pageContext.request.contextPath}/uploadExcelController/readExcel.do",		
		                ...
		                success: function(data) {   //提交成功后自动执行的处理函数，参数data就是服务器返回的数据。
		                	//$("#titleBox").html("");  
		                     alert(data.msg);
		                	 window.location.href="${pageContext.request.contextPath }/uploadExcelController/select.do?type=khhklr&dn=dh_repaymentEntry&qn=list&pagesize=10&pagenow=1";
