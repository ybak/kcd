...
<input type="hidden" value="5" id="colortype" name="colortype">
	<ul id="myTab" class="nav nav-tabs">
			<!-- <li>
			<a id="icbc1" style="background-color: #b1bacb;color: #ffffff;" href="newicbcsh.do?icbc_type=1&amp;id=442&amp;querytype=&amp;size=15&amp;status=">
			征信</a></li> -->
			<!--  
			<li ><a id="icbc2"  href="yp_sh.do?icbc_type=2&icbc_id=442&querytype=&size=15&status=" >预评</a></li>
			-->
			<!-- <li>
			<a id="icbc3" style="background-color: #b1bacb;color: #ffffff;" href="icbc_cars_sh.do?icbc_type=3&amp;icbc_id=442&amp;querytype=&amp;size=15&amp;status=">
			评估</a></li>
			<li>
			<a id="icbc4" style="background-color: #b1bacb;color: #ffffff;" href="icbc_sp_sh.do?icbc_type=4&amp;icbc_id=442&amp;querytype=&amp;size=15&amp;status=">
			视频</a></li> -->
			<li class="active"> 
			<a id="icbc5" style="background-color: rgb(255, 135, 0); color: rgb(255, 255, 255);" href="#">
			开卡</a></li>
			<!-- <li>
			<a id="icbc6" style="background-color: #b1bacb;color: #ffffff;" href="dk_sh.do?icbc_type=6&amp;icbc_id=442&amp;querytype=&amp;size=15&amp;status=">
			汽车贷款</a></li> -->
	       </ul>
  <div id="divmsg" class="admin-content nav-tabs-custom box">
  
  <!-- 
  <li ><a id="icbc6"  href="newicbcsh.do?icbc_type=7&id=442&querytype=&size=15&status=" >通融</a></li>
  <li ><a id="icbc6"  href="newicbcsh.do?icbc_type=8&id=442&querytype=&size=15&status=" >财务</a></li>	
  -->  
...
function kkup(){
	...
	$.ajax({
		   type:"post",   
		   url:"${pageContext.request.contextPath}/erp/kk_up.do",
		   data:form,
		   /**
...
<script>
$(function(){
	$("#kk_form").attr("onsubmit","return check()"); 
});
...
<a href="${pageContext.request.contextPath}/downloadFiles_all.do?type=kk&id=${requestScope.id }" class="btn btn-success" style="margin-left:100px">一键下载所有图片</a>
				...
				<div class="col-sm-12" style="margin-left:100px; margin-top: 50px;">
                    ...
                    <c:if test="${!empty imgs}">
						<div class="col-sm-2" style="margin:20px 10px;text-align: center;">
					        ...
					        <a style="padding: 0px 5px;" class="btn btn-warning" id="" href="${pageContext.request.contextPath}/downloadFile.do?fileUrl=http://a.kcway.net/assess/${imgs}&fileName=<%=m.get(i) %>">下载</a>							
			...
			<div class="row inline-from">
				<div class="col-sm-4">
					...
					<select name="bc_status" class="form-control" id="bc_status" onchange="autoremark();">
                            	...
                            	<option value="3"  ${requestScope.icbc_kk.bc_status==3?"selected='selected'":''}>身份核查通过-进入-开卡审核中</option>
...
$('#upload_result_pdf').on('change',function (){	
    ...
    $.ajax({
        url: "${pageContext.request.contextPath}/icbc_pdf.do",
        type: "POST",
        data: formData,
        /**
        *必须false才会自动加上正确的Content-Type
...
<c:forEach items="${requestScope.iResults }" var="il">${fn:substring(il.dt_add,0,19)}:状态：${il.statusremark},留言：${il.remark }
		...
		<div class="footer-wrap">
			<div class="box-footer" >
				<button type="button" class="btn btn-default" onclick="location.href='${pageContext.request.contextPath}/erp/index_.do?type=wlghd&dn=kk&qn=list&status=${requestScope.status}'">取消返回</button>
				<button type="button"  onclick="kkup();" class="btn btn-primary pull-right">保存提交</button> 
			</div>
		</div>	
</form>
