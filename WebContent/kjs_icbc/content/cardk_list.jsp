...
<script type="text/javascript">
    function a3(){
		...
		window.location.href ="${pageContext.request.contextPath}/erp/index_.do?type=wlghd&dn=cardk&qn=list&pagesize="+s+"&pagenow=1"+"&status="+status;
    }
    function a2(){
		...
		var querytype=$("#querytype").val();		
		window.location.href ="${pageContext.request.contextPath}/erp/index_.do?type=wlghd&dn=cardk&qn=list&pagesize="+s+"&pagenow=1"+"&status="+type_select;
    }
    function a1(){
		...
		window.location.href ="kjs_dk.do?querytype="+querytype+"&size="+s+"&status="+status;
    }
</script>				
		<section class="content-header">
			<h1>
			汽车贷款<small>共${requestScope.totalsize}个</small>				
			</h1>			
		</section>
		<!-- Main content -->
...
<!-- 							<a data-toggle="modal" data-target="#modal2" href="" style="background-color: #447feb;" class="btn btn-default">
				...
				<option value="3" ${requestScope.status==3?"selected='selected'":''}>订单结束(成功)</option>
				<option value="4" ${requestScope.status==4?"selected='selected'":''}>回退补件</option>
				<option value="5" ${requestScope.status==5?"selected='selected'":''}>订单结束(失败)</option>
				<option value="7" ${requestScope.status==7?"selected='selected'":''}>附件待审</option>
				<option value="8" ${requestScope.status==8?"selected='selected'":''}>附件回退</option>
			    </select>	
							</div>
					</div>
					<div class="col-sm-2 admin-page-top hidden-xs">
					   ...
					   <ul class="pagination no-margin">					     				       
						...
						<li>
							...
							%>
							<a href="${pageContext.request.contextPath }/erp/index_.do?type=wlghd&dn=cardk&qn=list&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}&status=" class="btn btn-default">«</a>						
							<%				    	
					         }						
							 if(pagenow1>=1&&pagenow1<totalpage1){
							%>
	...
	<tbody>
		...
		<%
		Map cardkmap=(Map)request.getAttribute("cardkmap");
		...
		<c:forEach var="pd" items="${requestScope.newpdList}">
			<%
			PageData pd=list.get(cardknub);
			%>
			
			<tr>
			<td class="text-center"><input name="delid" type="checkbox"></td>
			<td class="hidden-xs text-center">${pd.gems_code}</td>
			<td class="text-center">来源:${pd.fsname}-${pd.gemsname}</td>
			...
			<td class="text-center">
				...
				<a href="${pageContext.request.contextPath }/erp/wdrw_from.do?type=wlghd&dn=jdgz&qn=form&icbc_id=${pd.icbc_id}&cn=cardk" class="btn btn-default">
				<i class="fa fa-search-plus" title="明细查看"></i>
				</a>
				<a href="${pageContext.request.contextPath}/erp/index_.do?type=wlghd&dn=cardk&qn=form&id=${pd.id}&icbc_id=${pd.icbc_id}" class="btn btn-default">
				<i class="fa fa-pencil" title="审核编辑"></i>
				</a>
				</div>
			</td>
			<td><!--当前状态  -->
			<a href="${pageContext.request.contextPath}/erp/index_.do?type=wlghd&dn=cardk&qn=form&id=${pd.id}&icbc_id=${pd.icbc_id}">			
				<span id="dk${pd.id}"  class="label label-success">
					<c:if test="${pd.sh_status == 1}">专员</c:if>
					<c:if test="${pd.sh_status == 2}">主管</c:if>
					<c:if test="${pd.sh_status == 3}">经理</c:if>
					<c:if test="${pd.sh_status == 4}">总监</c:if>
					<%=cardkmap.get(Integer.parseInt(pd.get("bc_status").toString()))%>
				</span>
			</a>
			</td>
			</tr>
			...
			<c:if test="${requestScope.totalpage ge '1' }">
				<ul class="pagination no-margin">
				       <c:if test="${requestScope.pagenow ne '1' }">
				        <li><a href="${pageContext.request.contextPath }/erp/index_.do?type=wlghd&dn=cardk&qn=list&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}" aria-label="Next"><span aria-hidden="true">«</span></a></li>
				       </c:if>
					   <%
					   ...
					   <a id="a<%=j %>" href="${pageContext.request.contextPath }/erp/index_.do?type=wlghd&dn=cardk&qn=list&pagesize=${requestScope.pagesize}&pagenow=<%=j %>">
					   <%=j %>
					   </a>
					   </li>
					   <%
