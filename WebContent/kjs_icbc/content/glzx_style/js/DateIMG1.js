  ...
  //通用样式
   ...
   $.ajax({
 		...
 		success : function(data) {
 			 var summarydata = [6139,6292,6412,4708,8477,7983,4916,5102,148];
...
/*过件率(汽车贷款通过)start*/
   ...
   $.ajax({
 		...
 		success : function(data) {
 			 var carpassdata = [4473,4866,4992,3449,6521,5665,3569,3705,106];
	...
	var fangkkuan_lineANDbar_3 = echarts.init(document.getElementById('fangkkuan_lineANDbar_3'));
	   ...
	   $.ajax({
			...
			success : function(data) {
				   ...
				   var oldcars = [4529,3426,3947,4026,4380,4493,3105,5869,5009, 3213,3396,91];
				   var oldcarsmoney = [33520.049,25696.826,31787.297,31405.625,34606.016,35043.983,22978.304,47482.919,38245.868,24341.129,25599.71,744.33];
	             if(time == "2018"){
...
$.ajax({
		...
		success : function(data) {
		   var recycledata = [4373,4766,4892,3349,6421,5565,3469,3705,106];
...
function yuqiselect() {
    ...
    $.ajax({
        ...
        success: function (data) {
        	var amount = [0.21,0.16,0.10];
            var newcars = [0,0,0];
            var oldcars = [0.21,0.16,0.10];
            var amountmoney = [1.69,1.27,0.85];
            var newcarsmoney = [0,0,0];
            var oldcarsmoney = [1.69,1.27,0.85];
 option_yuqilv1 = {
    tooltip: { //提示框组件。 
        trigger: 'axis',//触发类型:'axis'坐标轴触发，主要在柱状图，折线图等会使用类目轴的图表中使用。
        axisPointer: { //坐标轴指示器配置项。
 ...
 var zijinzhouzhuanlv = echarts.init(document.getElementById('zijinzhouzhuanlv'));
	...
	$.ajax({
		...
		success : function(data) {
		   ...
		   var advancefunddata = [3.25,4.09,3.65,3.35,3.12,2.98,2.65,3.04,3.87,3.25,2.58,3.58];
