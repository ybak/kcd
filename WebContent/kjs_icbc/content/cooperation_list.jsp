...
<script type="text/javascript">
    function a3(){
		...
		window.location.href ="${pageContext.request.contextPath}/lendingResultController/select.do?type=dhywgl3&dn=cooperation&qn=list&pagesize="+s+"&pagenow=1";
    }
    function reset(){
    	$("#myform")[0].reset()
    }
		...
		<input type="hidden" id="size" name="size" value="${reqeustScope.pagesize}">
			<div class="admin-tools">
				<div class="row">
					<div class="col-sm-10">
					<div class="btn-group">
						<form name="myform" class="form-horizontal" action="${pageContext.request.contextPath }/lendingResultController/select.do?type=dhywgl3&dn=cooperation&qn=list&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}" enctype="multipart/form-data" method="post">
							<label>搜索功能</label>
					...
					<li>
						...
						%>
						<a href="${pageContext.request.contextPath }/lendingResultController/select.do?type=dhywgl3&dn=cooperation&qn=list&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}" class="btn btn-default">«</a>						
						<%				    	
				         }						
						 if(pagenow1>=1&&pagenow1<totalpage1){
						%>
	...
	<tbody>
		...
		<tr>
			...
			<td class="text-center">
				<a href="${pageContext.request.contextPath }/lendingResultController/selectdetail.do?type=dhywgl3&dn=cooperation&qn=form&c_cardno=${l1.c_cardno}">
			...
			<c:if test="${requestScope.totalpage ge '1' }">
				<ul class="pagination no-margin">
				       <c:if test="${requestScope.pagenow ne '1' }">
				        <li><a href="${pageContext.request.contextPath }/lendingResultController/select.do?type=dhywgl3&dn=cooperation&qn=list&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}" aria-label="Next"><span aria-hidden="true">«</span></a></li>
				       </c:if>
					   <%
					   ...
					   <a id="a<%=j %>" href="${pageContext.request.contextPath }/lendingResultController/select.do?type=dhywgl3&dn=cooperation&qn=list&pagesize=${requestScope.pagesize}&pagenow=<%=j %>">
					   <%=j %>
					   </a>
					   </li>
					   <%
