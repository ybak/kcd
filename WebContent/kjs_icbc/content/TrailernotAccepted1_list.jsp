...
<script type="text/javascript">
    function a3(){
		...
		window.location.href ="${pageContext.request.contextPath}/trailernotAcceptedController/select1.do?type=tc_ysl&dn=TrailernotAccepted1&qn=list&pagesize="+s+"&pagenow=1";
    }
    function reset(){
    	$("#myform")[0].reset()
    }
		...
		<input type="hidden" id="size" name="size" value="${reqeustScope.pagesize}">
			<div class="admin-tools">
				<div class="row">
					<div class="col-sm-10">
						<div class="btn-group">		
							<form name="myform" class="form-horizontal" action="${pageContext.request.contextPath}/trailernotAcceptedController/select1.do?type=tc_ysl&dn=TrailernotAccepted1&qn=list" enctype="multipart/form-data" method="post">
					...
					<div class="col-sm-2">
					   ...
					   <ul class="pagination no-margin">					     				       
						...
						<li>
							...
							%>
							<a href="${pageContext.request.contextPath }/trailernotAcceptedController/select1.do?type=tc_ysl&dn=TrailernotAccepted1&qn=list&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}" class="btn btn-default">«</a>						
							<%				    	
					         }						
							 if(pagenow1>=1&&pagenow1<totalpage1){
							%>
		...
		<div id="main_list" class="admin-content box">
				...
				<tbody>	
					...
					<c:forEach var="newpdList" items="${newpdList}">
						...
						<td class="text-center">
							${newpdList.loans_bank }
						</td>
						<td class="text-center">
							<c:if test="${newpdList.cars_type == '1'}">
								新车
							</c:if>
							<c:if test="${newpdList.cars_type == '0'}">
								二手车
							</c:if>
						</td>
						<td class="text-center">
						...
						<td class="text-center">
							${newpdList.gems_fs_name }
						</td>
						<td class="text-center">
							${newpdList.cars_tel }
						</td>
						<td class="text-center">
							<c:if test="${newpdList.status == '0'}">
								未处理
							</c:if>
							<c:if test="${newpdList.status == '1'}">
								已处理
							</c:if>
							<c:if test="${newpdList.status == '2'}">
								完成
							</c:if>
						</td>
						<td class="text-center">
							
							<a href="${pageContext.request.contextPath }/trailernotAcceptedController/selectdetail1.do?type=tc_ysl&dn=TrailernotAccepted1&qn=form&icbc_id=${newpdList.icbc_id}">
			...
			<c:if test="${requestScope.totalpage ge '1' }">
				<ul class="pagination no-margin">
				       <c:if test="${requestScope.pagenow ne '1' }">
				        <li><a href="${pageContext.request.contextPath }/trailernotAcceptedController/select1.do?type=tc_ysl&dn=TrailernotAccepted1&qn=list&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}" aria-label="Next"><span aria-hidden="true">«</span></a></li>
				       </c:if>
					   <%
					   ...
					   <a id="a<%=j %>" href="${pageContext.request.contextPath }/trailernotAcceptedController/select1.do?type=tc_ysl&dn=TrailernotAccepted1&qn=list&pagesize=${requestScope.pagesize}&pagenow=<%=j %>">
					   <%=j %>
					   </a>
					   </li>
					   <%
