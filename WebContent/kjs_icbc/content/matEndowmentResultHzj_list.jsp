...
<div class="col-sm-10">
<div class="btn-group">
<form id="search_form" class="form-horizontal" action="${pageContext.request.contextPath}/erp/matEndowentResult_list.do">
...
<label>搜索功能</label>
<input placeholder="请输入业务编号或客户姓名或身份证号" style="width: 300px;" value="" class="form-control" id="selectMsg" name="selectMsg"/>
<label >是否垫资</label>
<select style="width: 200px;" class="form-control" id="assess_cp" name="assess_cp">
<option value="0">--请选择--</option>
<option value="1" ${requestScope.assess_cp eq '1'?"selected='selected'":'' }>未垫资</option>
...
laydate.render({
  elem: '#date2' //指定元素
			...
			<div class="admin-tools">
				<div class="row">
					...
					<div class="col-sm-2 admin-page-top hidden-xs">
							<select id="page_limit_select" onchange="window.location.href='${pageContext.request.contextPath }/erp/matEndowentResult_list.do?type=dqywgl7&dn=matEndowmentResultHzj&cn=${requestScope.cn}&qn=list&selectMsg=${requestScope.selectMsg}&assess_cp=${requestScope.assess_cp}&pagenow=${requestScope.pagenow}&pagesize='+this.value+'&fsid=1'" class="form-control">
		...
		<div class="btn-group">
						...
						%>
						<a href="${pageContext.request.contextPath }/erp/matEndowentResult_list.do?type=dqywgl7&dn=matEndowmentResultHzj&cn=${requestScope.cn }&qn=list&selectMsg=${requestScope.selectMsg}&assess_cp=${requestScope.assess_cp}&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}&fsid=${requestScope.fsid }" class="btn btn-default">«</a>						
						<%				    	
				         }						
						 if(pagenow1>=1&&pagenow1<totalpage1){
						%>
	...
	<tbody>
		<tr>
			...
			<th class="hidden-xs text-center" style="width:100px">垫资金额</th>
			<th class="hidden-xs text-center" style="width:100px">垫资机构</th>
			<th class="text-center" style="width:100px">垫资日期</th>
			<th class="hidden-xs text-center" style="width:100px">垫资状态</th>
		...
		<tr>
			...
			<td class="text-center">${pd.c_cardno}</td>
			<td class="hidden-xs text-center">${pd.rz_dzje}</td>
			<td class="hidden-xs text-center">${pd.rz_bank}</td>
			<td class="hidden-xs text-center">${pd.rz_date}</td>
			<td class="text-center">
				<c:if test="${pd.status==1}">
					<span class="label label-danger">未垫资</span>
				</c:if>
				<c:if test="${pd.status==2}">
					<span class="label label-success">已垫资</span>
				</c:if>
			</td>
			<%-- 
			<td class="text-center">
			...
			<c:if test="${requestScope.totalpage ge '1' }">
				<ul class="pagination no-margin">
				       <c:if test="${requestScope.pagenow ne '1' }">
				        <li><a href="${pageContext.request.contextPath }/erp/matEndowentResult_list.do?type=dqywgl7&dn=matEndowmentResultHzj&cn=${requestScope.cn }&qn=list&selectMsg=${requestScope.selectMsg}&assess_cp=${requestScope.assess_cp}&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}&fsid=${requestScope.fsid }" aria-label="Next"><span aria-hidden="true">«</span></a></li>
				       </c:if>
					   <%
					   ...
					   <a id="a<%=j %>" href="${pageContext.request.contextPath }/erp/matEndowentResult_list.do?type=dqywgl7&dn=matEndowmentResultHzj&cn=${requestScope.cn }&qn=list&selectMsg=${requestScope.selectMsg}&assess_cp=${requestScope.assess_cp}&pagesize=${requestScope.pagesize}&pagenow=<%=j %>&fsid=${requestScope.fsid }">
					   <%=j %>
					   </a>
					   </li>
					   <%
