...
<script type="text/javascript">
    function a3(){
		...
		window.location.href ="${pageContext.request.contextPath}/erp/index_.do?type=wlghd&dn=mq&qn=list&pagesize="+s+"&pagenow=1"+"&status="+status;
    }
    function a2(){
		...
		var querytype=$("#querytype").val();		
		window.location.href ="${pageContext.request.contextPath}/erp/index_.do?type=wlghd&dn=mq&qn=list&pagesize="+s+"&pagenow=1"+"&status="+type_select;
    }
    function a1(){
		...
		window.location.href ="kjs_mq.do?querytype="+querytype+"&size="+s+"&status="+status;
    }
</script>				
		<section class="content-header">
			<h1>
			视频面签<small>共${requestScope.totalsize}个</small>						
			</h1>			
		</section>
		<!-- Main content -->
					...
					<div class="col-sm-2 admin-page-top hidden-xs">
					   ...
					   <ul class="pagination no-margin">					     				       
						...
						<li>
							...
							%>
							<a href="${pageContext.request.contextPath }/erp/index_.do?type=wlghd&dn=mq&qn=list&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}" class="btn btn-default">«</a>						
							<%				    	
					         }						
							 if(pagenow1>=1&&pagenow1<totalpage1){
							%>
	...
	<tbody>
		...
		<c:forEach var="pd" items="${requestScope.newpdList}">
			...
			<td class="text-center">
				...
				<a href="${pageContext.request.contextPath }/erp/wdrw_from.do?type=wlghd&dn=jdgz&qn=form&icbc_id=${pd.icbc_id}&cn=mq" class="btn btn-default">
				<i class="fa fa-search-plus" title="明细查看"></i>
				</a>
				<a href="${pageContext.request.contextPath}/erp/index_.do?type=wlghd&dn=mq&qn=form&id=${pd.id}&icbc_id=${pd.icbc_id}" class="btn btn-default">
				<i class="fa fa-pencil" title="审核编辑"></i>
				</a>
				</div>
			</td>
			<td><!--当前状态  -->
			<a href="${pageContext.request.contextPath}/erp/index_.do?type=wlghd&dn=mq&qn=form&id=${pd.id}&icbc_id=${pd.icbc_id}">
			<span id="mq${pd.id }"  class="label label-success"><%=mqmap.get(Integer.parseInt(pd.get("bc_status").toString())) %></span></a>
			</td>	
			</tr>
			<% mqnub++; %>
			</c:forEach>
			...
			<c:if test="${requestScope.totalpage ge '1' }">
				<ul class="pagination no-margin">
				       <c:if test="${requestScope.pagenow ne '1' }">
				        <li><a href="${pageContext.request.contextPath }/erp/index_.do?type=wlghd&dn=mq&qn=list&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}" aria-label="Next"><span aria-hidden="true">«</span></a></li>
				       </c:if>
					   <%
					   ...
					   <a id="a<%=j %>" href="${pageContext.request.contextPath }/erp/index_.do?type=wlghd&dn=mq&qn=list&pagesize=${requestScope.pagesize}&pagenow=<%=j %>">
					   <%=j %>
					   </a>
					   </li>
					   <%
