...
<script type="text/javascript">
    function a3(){
		...
		window.location.href ="${pageContext.request.contextPath}/lendingResultController/select.do?type=wlghd&dn=lendingResult&qn=list&pagesize="+s+"&pagenow=1";
    }
    function reset(){
    	$("#myform")[0].reset()
    }
			...
			<div class="admin-tools">
				<div class="row">
					...
					<li>
						...
						%>
						<a href="${pageContext.request.contextPath }/lendingResultController/select.do?type=wlghd&dn=lendingResult&qn=list&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}" class="btn btn-default">«</a>						
						<%				    	
				         }						
						 if(pagenow1>=1&&pagenow1<totalpage1){
						%>
	...
	<tbody>
		...
		<tr>
			...
			<td class="text-center">
				<a href="${pageContext.request.contextPath }/lendingResultController/selectdetail.do?type=wdrw&dn=lendingResult&qn=form&cn=w1&c_cardno=${l1.c_cardno}">
			...
			<c:if test="${requestScope.totalpage ge '1' }">
				<ul class="pagination no-margin">
				       <c:if test="${requestScope.pagenow ne '1' }">
				        <li><a href="${pageContext.request.contextPath }/lendingResultController/select.do?type=wlghd&dn=lendingResult&qn=list&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}" aria-label="Next"><span aria-hidden="true">«</span></a></li>
				       </c:if>
					   <%
					   ...
					   <a id="a<%=j %>" href="${pageContext.request.contextPath }/lendingResultController/select.do?type=wlghd&dn=lendingResult&qn=list&pagesize=${requestScope.pagesize}&pagenow=<%=j %>">
					   <%=j %>
					   </a>
					   </li>
					   <%
