...
<script type="text/javascript">
    function a3(){
		...
		window.location.href ="${pageContext.request.contextPath}/lawsuitController/select1.do?type=${param.type }&dn=lawsuit1&qn=list&pagesize="+s+"&pagenow=1";
    }
    function reset(){
    	$("#myform")[0].reset()
    }
		...
		<input type="hidden" id="size" name="size" value="${reqeustScope.pagesize}">
			<div class="admin-tools">
				<div class="row">
					<div class="col-sm-10">
						<div class="btn-group">		
							<form name="myform" class="form-horizontal" action="${pageContext.request.contextPath}/lawsuitController/select1.do?type=${param.type }&dn=lawsuit1&qn=list" enctype="multipart/form-data" method="post">
					...
					<div class="col-sm-2">
					   ...
					   <ul class="pagination no-margin">					     				       
						...
						<li>
							...
							%>
							<a href="${pageContext.request.contextPath }/lawsuitController/select1.do?type=${param.type }&dn=lawsuit1&qn=list&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}" class="btn btn-default">«</a>						
							<%				    	
					         }						
							 if(pagenow1>=1&&pagenow1<totalpage1){
							%>
				...
				<tbody>	
					...
					<c:forEach var="newpdList" items="${newpdList}">
						...
						<td class="text-center">
							...
							<a href="${pageContext.request.contextPath }/lawsuitController/selectdetail1.do?type=${param.type }&dn=lawsuit1&qn=form&icbc_id=${newpdList.icbc_id}">
			...
			<c:if test="${requestScope.totalpage ge '1' }">
				<ul class="pagination no-margin">
				       <c:if test="${requestScope.pagenow ne '1' }">
				        <li><a href="${pageContext.request.contextPath }/lawsuitController/select1.do?type=${param.type }&dn=lawsuit1&qn=list&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}" aria-label="Next"><span aria-hidden="true">«</span></a></li>
				       </c:if>
					   <%
					   ...
					   <a id="a<%=j %>" href="${pageContext.request.contextPath }/lawsuitController/select1.do?type=${param.type }&dn=lawsuit1&qn=list&pagesize=${requestScope.pagesize}&pagenow=<%=j %>">
					   <%=j %>
					   </a>
					   </li>
					   <%
