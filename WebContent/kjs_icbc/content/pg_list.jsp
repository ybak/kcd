...
<script type="text/javascript">
    function a3(){
		...
		window.location.href ="${pageContext.request.contextPath}/erp/index_.do?type=wlghd&dn=pg&qn=list&pagesize="+s+"&pagenow=1"+"&status="+status;
    }
    function a2(){
		...
		var querytype=$("#querytype").val();		
		window.location.href ="${pageContext.request.contextPath}/erp/index_.do?type=wlghd&dn=pg&qn=list&pagesize="+s+"&pagenow=1"+"&status="+type_select;
    }
    function a1(){
		...
		<section class="content-header">
			<h1>
			汽车评估<small>共${requestScope.totalsize}个</small>	
			</h1>			
		</section>
		<!-- Main content -->
		...
		<input type="hidden" id="size" name="size" value="${requestScope.pagesize}">
...
<!-- 							<a data-toggle="modal" data-target="#modal2" href="" style="background-color: #447feb;" class="btn btn-default">
					...
					<div class="col-sm-2 admin-page-top hidden-xs">
					   ...
					   <ul class="pagination no-margin">					     				       
						...
						<li>
							...
							%>
							<a href="${pageContext.request.contextPath }/erp/index_.do?type=wlghd&dn=pg&qn=list&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}" class="btn btn-default">«</a>						
							<%				    	
					         }						
							 if(pagenow1>=1&&pagenow1<totalpage1){
							%>
	...
	<tbody>
		...
		%>
			...
			<td class="hidden-xs text-center">${pd.c_name}</td>
			<td class="hidden-xs text-center">${pd.c_carno}</td>
			<td class="hidden-xs text-center">
			<fmt:formatNumber type="number" value="${pd.icbc_pricecs}" pattern="0.00" maxFractionDigits="2"/>
			</td>
			<td class="hidden-xs text-center">
			<fmt:formatNumber type="number" value="${pd.price_result}" pattern="0.00" maxFractionDigits="2"/>
			</td>
			<td class="hidden-xs text-center">${pd.c_name}</td>
			<td class="text-center">
				<!--  编辑-->
				<div class="table-button">
				<a href="${pageContext.request.contextPath }/erp/wdrw_from.do?type=wlghd&dn=jdgz&qn=form&icbc_id=${pd.icbc_id}&cn=pg" class="btn btn-default">
				<i class="fa fa-search-plus" title="明细查看"></i>
				</a>
				<a href="${pageContext.request.contextPath}/erp/index_.do?type=wlghd&dn=pg&qn=form&id=${pd.id}&icbc_id=${pd.icbc_id}" class="btn btn-default">
			...
			<td><!--当前状态  -->
			<a href="${pageContext.request.contextPath}/erp/index_.do?type=wlghd&dn=pg&qn=form&id=${pd.id}&icbc_id=${pd.icbc_id}">
			<span id="pg${pd.id}"  class="label label-success" ><%=pgmap.get(Integer.parseInt(pd.get("bc_status").toString())) %></span>
			</a>
			</td>
			</tr>
			<% pgnub++; %>
			...
			<c:if test="${requestScope.totalpage ge '1' }">
				<ul class="pagination no-margin">
				       <c:if test="${requestScope.pagenow ne '1' }">
				        <li><a href="${pageContext.request.contextPath }/erp/index_.do?type=wlghd&dn=pg&qn=list&pagesize=${requestScope.pagesize}&pagenow=${requestScope.pagenow-1}" aria-label="Next"><span aria-hidden="true">«</span></a></li>
				       </c:if>
					   <%
					   ...
					   <a id="a<%=j %>" href="${pageContext.request.contextPath }/erp/index_.do?type=wlghd&dn=pg&qn=list&pagesize=${requestScope.pagesize}&pagenow=<%=j %>">
					   <%=j %>
					   </a>
					   </li>
					   <%
