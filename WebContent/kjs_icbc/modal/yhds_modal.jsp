	...
	<c:if test="${erp15.status eq '14'}">
 	 ...
 	 <c:if test="${erp15.status eq '15'}">
		...
		<form name="modalForm" class="form-horizontal ng-pristine ng-valid ng-scope" >
			<!-- ngIf: notUseButton --><div class="form-group ng-scope" >
			...
			<div style="display:none;">
					<textarea id="inObj">${erp15.result_1_value}</textarea>
			</div>
			<div class="form-group">
				<label class="col-sm-2 control-label" style="margin-top: 57px;">电审发起说明</label>
				<div class="col-sm-8" style="margin-top: 40px;">
					<textarea id="texta" name="texta" rows="3" class="form-control ng-pristine ng-untouched ng-valid ng-empty" type="text">
					</textarea>
				</div>
			</div> 
	<div class="form-group">
...
${erp15.result_1_msg }
			...
			<script>
				// 处理 json ,并给相应的对象赋值
	        	var objS = JSON.parse($("#inObj").val()); //由JSON字符串转换为JSON对象
	    		document.getElementById("texta").innerHTML = objS.dsfqsm;
			</script>
		</form>
		</div>                                             
		</div>							  	
