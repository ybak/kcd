...
%>
<c:if  test="${fn:contains(sessionScope.pd.purview_map,'lryhcyqk_81')==true}">
...
<form id="erp_form" name="erp_form" class="form-horizontal ng-pristine ng-valid ng-scope ng-valid-required ng-valid-pattern ng-valid-maxlength">
    ...
    <input id="status_id" name="status_id" value="81" type="hidden" />
...
function erp(){
	...
	var val=$('input:radio[name="cyqk"]:checked').val();
	if(cphm==""){
		alert("请输入车牌号码!");
	}else if(dywcrq==""){
		alert("请输入抵押完成日期!");
		...
		$.ajax({
	        url:"${pageContext.request.contextPath }/erp/erp_dygd_81.do",
	        type:"post",
	        data:form,
	        processData:false,
	        contentType:false,
