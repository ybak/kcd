...
function a3(){
	...
	//alert(s);
	window.location.href ="dsjlist.do?size="+s;
}

function a1(size) {
	alert(size);
	window.location.href ="dsjlist.do?size="+size+"&pagenow=3";
}
</script>	
...
$('li.active').parents('li').addClass('treeview').addClass('active');
	...
	<tbody>
		<tr>
			...
			<th class="hidden-xs ext-center" style="width:80px">结束时间</th>
			<th style="width: 120px" class="text-center">类型</th>
			<th style="width: 80px" class="text-center">查询状态</th>		
			</tr>
				
			<c:forEach items="${requestScope.dsjlist }" var="dsj" >										
			<tr>
			<td class="hidden-xs">${dsj.gsname }-${dsj.ryname }</td>
			<td class="hidden-xs">${dsj.name }</td>
			<td class="hidden-xs">-10.00元</td>
			<td class="hidden-xs">${dsj.idcard }</td>
			<td class="hidden-xs">${dsj.phonenum }</td>
			<td class="hidden-xs">${dsj.time }</td>
			<td class="hidden-xs">${dsj.uptime }</td>
			<td class="text-center">		
			<span class="label label-danger">API大数据查询</span>			
			</td>
			<td class="text-center"><span class="label label-success">查询完成</span>
				...
				<ul class="pagination no-margin">
				       ...
				       <li >
				       <a  href="dsjlist.do?size=${requestScope.size }&pagenow=${requestScope.pagenow-1}">
				       <span >«</span>
				       </a>
				       </li>
				       </c:if>
					   ...
					   <a id="a<%=j %>" href="dsjlist.do?size=${requestScope.size }&pagenow=<%=j %>">
					   <%=j %>
					   </a>
					   </li>
					   <%
					   ...
					   %>
	                   <li id="l<%=j %>" >
					   <a id="a<%=j %>" href="dsjlist.do?size=${requestScope.size }&pagenow=<%=j %>"><%=j %>					   
					   </a>
					   </li>					   
		               <%
				       }
