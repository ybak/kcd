...
<meta name="renderer" content="webkit|ie-comp|ie-stand">
	<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
		...
		<![endif]-->
			...
			<script src="${pageContext.request.contextPath}/wangye0119/js/H-ui.js" type="text/javascript"></script>
			<script src="${pageContext.request.contextPath}/wangye0119/js/displayPart.js" type="text/javascript"></script>
			<title>文章管理</title>
</head>
				...
				<div class="widget-box side_content">
					...
					<div class="side_list">
						<div class="widget-header header-color-green2">
							<h4 class="lighter smaller">团队列表</h4>
						</div>
						<c:choose>
							<c:when test="${sessionScope.bms_deptno==10}">
								<div class="widget-body">
									<ul class="b_P_Sort_list">
										<li><i class="orange  fa fa-list "></i><a href="${pageContext.request.contextPath}/selectLoginUserTeam.do?upid=${sessionScope.bms_id}">团队个数(${requestScope.managerSum})</a></li>
										<c:forEach items="${requestScope.managerList}" var="manager">
											<li><i class="fa fa-shopping-bag pink "></i><a href="${pageContext.request.contextPath}/selectLoginUserTeam.do?upid=${manager.uid}">${manager.uname}-团队</a></li>
										</c:forEach>
									</ul>
								</div>
							</c:when>
							<c:when test="${sessionScope.bms_deptno==40}">
								<div class="widget-body">
									<ul class="b_P_Sort_list">
										<li><i class="orange  fa fa-list "></i><a href="${pageContext.request.contextPath}/selectLoginUserTeam.do?upid=${sessionScope.bms_id}">团队个数(${requestScope.managerSum})</a></li>
										<c:forEach items="${requestScope.managerList}" var="manager">
											<li><i class="fa fa-shopping-bag pink "></i><a href="${pageContext.request.contextPath}/selectLoginUserTeam.do?upid=${manager.uid}">${manager.uname}-团队-${manager.uid}(${requestScope.managetCountSum})</a></li>
										</c:forEach>
									</ul>
								</div>
							</c:when>
			...
			<div class="Ads_list">
				...
				<div class="article_list">
					<table class="table table-striped table-bordered table-hover"
						...
						<tbody>
							<c:forEach items="${requestScope.list}" var="subordinate" varStatus="vs">
								<tr>
									<td><label><input type="checkbox" class="ace"><span
												class="lbl"></span></label></td>
									<td>${vs.index+1}</td>
									<td>
										<a href="${pageContext.request.contextPath}/selectOtherUserClient.do?ucid=${subordinate.uid}">${subordinate.uname}</a>
									</td>
									<td>${subordinate.username}</td>
									<td>${subordinate.userPhone}</td>
									<td>${subordinate.regTime}</td>
									<td class="td-manage"><a title="删除" href="javascript:;"
										onclick="member_del(this,'${subordinate.uid}',${requestScope.upid})" class="btn btn-xs btn-danger"><i
...
/*文章-删除*/
function member_del(obj,id,upid){
	layer.confirm('确认要删除吗？',function(index){
		$(obj).parents("tr").remove();
		layer.msg('已删除!',{icon:1,time:1000});
		location.href="${pageContext.request.contextPath}/deleteManagerUserByUid.do?uid="+id+"&upid="+upid;
	});
}
