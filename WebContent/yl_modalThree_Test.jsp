...
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
 <div id="float_page_div">
	...
	<input name="qryid" value="470" type="hidden">
	 <input name="type" value="9" type="hidden">
<input name="gems_fs_id" value="1445" type="hidden">
<input name="bank_id" value="61" type="hidden">
<input name="ds_type" value="0" type="hidden">
<input name="c_name" value="" type="hidden">
<input name="c_cardno" value="" type="hidden">
<input name="sms_tel" value="" type="hidden">
	<div class="form-group" style="padding-top: 25px">
		<label for="title2" class="col-sm-3 control-label">签约银联卡:</label>
		<div class="col-sm-9">
			<input value="${requestScope.khName}-${requestScope.khCardId}<%-- ${ydd.c_name}-${ydd.c_cardno} --%>" class="form-control" id="bt" disabled="disabled" type="text">
		</div>
	</div>
	<div class="form-group">
		...
		<c:choose>			
			<c:when test="${requestScope.status == 3}">
				<input value="30000000" class="form-control" name="AMOUNT" id="AMOUNT" placeholder="输入代收金额,单位(分)"  type="number">						
			</c:when>
			<c:otherwise>	 
				<input value="${ydd.AMOUNT}" class="form-control" name="AMOUNT" id="AMOUNT" placeholder="输入代收金额,单位(分)"  type="number">
			</c:otherwise>
		</c:choose>
		</div>
