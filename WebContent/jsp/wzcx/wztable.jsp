...
function a2(){
	...
	//alert(s)
	window.location.href ="wzlist.do?pagesize="+s;
...
<!-- 	<body class="skin-green sidebar-mini fixed"> -->
				...
				<div class="row">
					...
					<div class="col-sm-5 admin-page-top hidden-xs">
							<select id="page_limit_select" onchange="a2()" class="form-control">
							                            <option value="10">当前每页显示${requestScope.pagesize }条</option>
	...
	<tbody>
		<tr>
			<th>公司名称|姓名</th>
			<th class="hidden-xs">查询类型</th>
			<th class="hidden-xs">费用/次</th>
			<th class="text-center">添加时间</th>
			<th class="text-center">最后更新时间</th>			
			<th style="width: 150px" class="text-center">操作</th>
			<th style="width: 150px" class="text-center">状态</th>
	    </tr>
		  	<c:forEach var="l" items="${requestScope.list}">		  			  								
			<tr>
			<!-- <td class="text-center hidden-xs"><input type="checkbox" name="delid" value="37581"></td> -->
			<td>来源:${l.gname }-${l.pname }</td>
			<td>全国违章查询</td>
			<td>${l.price }元</td>
			<td class="text-center">
			${fn:substring(l.addtime, 0,19)}
			</td>
			<td class="text-center">
			${fn:substring(l.uptime, 0,19)}
			</td>
            <td class="text-center">
				<div class="table-button">
				<a href="wzsh.do?id=${l.id }" class="btn btn-default">
				<i class="fa fa-pencil"></i>
				</a>
                </div>
		    </td>
		    <td class="text-center">
		    <c:if test="${l.orderstate eq '1'}">
		    <span class="label label-success">查询完成</span>
		    </c:if>
		    <c:if test="${l.orderstate eq '0'}">
...
$("#menu li").hover(function() {
    ...
    $(this).removeClass("active");
				...
				<ul class="pagination no-margin">
				       ...
				       <li >
				       <a  href="wzlist.do?pagesize=${requestScope.pagesize }&pagenow=${requestScope.now-1}">
				       <span >«</span>
				       </a>
				       </li>
				       </c:if>
					   ...
					   <a id="a<%=j %>" href="wzlist.do?pagesize=${requestScope.pagesize }&pagenow=<%=j %>">
					   <%=j %>
					   </a>
					   </li>
					   <%
					   ...
					   %>
	                   <li id="l<%=j %>" >
					   <a id="a<%=j %>" href="wzlist.do?pagesize=${requestScope.pagesize }&pagenow=<%=j %>"><%=j %>					   
					   </a>
					   </li>					   
		               <%
				       }
				...
				<div class="page-num">共${requestScope.totalCount }个 分${requestScope.totalpage }页显示</div>
			</div>
  </section><!-- /.content -->
		</body>
