...
function a2(){
	...
	//alert(s)
	window.location.href ="apiuserlist.do?size="+s;
...
<!-- 	<body class="skin-green sidebar-mini fixed"> -->
	...
	<tbody>
		<tr>
			<th>公司名称</th>
			<th class="hidden-xs">公司地址</th>
			<th class="text-center">时间</th>
			<th style="width: 150px" class="text-center">客户姓名</th>
			<th style="width: 150px" class="text-center">客户手机号</th>
			<th style="width: 150px" class="text-center">客户身份证号</th>
			<th style="width: 150px" class="text-center">账户余额</th>
			<th style="width: 150px" class="text-center">APPKEY</th>
			<th style="width: 150px" class="text-center">操作</th>
	    </tr>
		  	<c:forEach var="au" items="${requestScope.apiuser}">		  			  								
			<tr>
			<!-- <td class="text-center hidden-xs"><input type="checkbox" name="delid" value="37581"></td> -->
			<td>${au.api_companyname }</td>
			<td class="hidden-xs">${au.api_companyaddress }</td>
			<td class="text-center">
			添加时间：${fn:substring(au.api_addtime, 0,19)}<br/>
			更新时间：${fn:substring(au.api_uptime, 0,19)}
			</td>
			<td class="text-center" >${au.api_name }</td>
			<td class="text-center" >${au.api_tel }</td>
			<td class="text-center" >${au.api_card }</td>
			<td class="text-center" >${au.api_money }元</td>
			<td class="text-center" >${au.appkey }</td>
            <td class="text-center">
				<div class="table-button">
				<a href="apiuserup.do?id=${au.id }" class="btn btn-default">
...
<c:if test="${requestScope.totalall ge '1' }">
				<ul class="pagination no-margin">
				       ...
				       <li >
				       <a  href="apiuserlist.do?pagesize=${requestScope.size }&pagenow=${requestScope.pagenow-1}">
				       <span >«</span>
				       </a>
				       </li>
				       </c:if>
					   ...
					   <a id="a<%=j %>" href="apiuserlist.do?pagesize=${requestScope.size }&pagenow=<%=j %>">
					   <%=j %>
					   </a>
					   </li>
					   <%
					   ...
					   %>
	                   <li id="l<%=j %>" >
					   <a id="a<%=j %>" href="apiuserlist.do?pagesize=${requestScope.size }&pagenow=<%=j %>"><%=j %>					   
					   </a>
					   </li>					   
		               <%
				       }
