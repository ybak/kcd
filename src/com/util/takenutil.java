...
public class takenutil {
    static Logger log = LoggerFactory.getLogger(takenutil.class);
    ...
    public takenutil(String ak, String sk) {
        this.accessKey = ak;
        this.secretKey = sk;
    }
    ...
    public String generateToken(String urlPath, String method, String queryParam, String body, int expireTime) {
        ...
        try {
            ...
            // {UrlPath}|
            sbSign.append(takenutil.decodeUtf8(urlPath)).append("|");
            ...
            if (queryParam != null && !queryParam.isEmpty()) {
                ...
                for (String kv : queryParam.split("&")) {
                    String[] t = kv.split("=");
                    if (t.length > 1) {
                        qsArray.add(String.format("%s=%s", takenutil.decodeUtf8(t[0]), takenutil.decodeUtf8(t[1])));
                    } else {
                        qsArray.add(String.format("%s=", takenutil.decodeUtf8(t[0])));
                    }
                }
            ...
            token = String.format("%s-%s-%s-%s", TOKEN_VERSION, accessKey, expireTime,
                    new String(takenutil.encodeHex(digest.digest())));
        } catch (Exception e) {
