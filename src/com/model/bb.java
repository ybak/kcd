package com.model;

public class bb {
  private int bid;
  private String khxm;
  private String sfzbh;
  private String md;
  private String sqsbh;
  private String orderid;
  private String cxr;
  private String stime;
  private String bb;
  private String onecxtime;
  private String cxerror;
  private String errortest;
public int getBid() {
	return bid;
}
public void setBid(int bid) {
	this.bid = bid;
}
public String getKhxm() {
	return khxm;
}
public void setKhxm(String khxm) {
	this.khxm = khxm;
}
public String getSfzbh() {
	return sfzbh;
}
public void setSfzbh(String sfzbh) {
	this.sfzbh = sfzbh;
}
public String getMd() {
	return md;
}
public void setMd(String md) {
	this.md = md;
}
public String getSqsbh() {
	return sqsbh;
}
public void setSqsbh(String sqsbh) {
	this.sqsbh = sqsbh;
}
public String getOrderid() {
	return orderid;
}
public void setOrderid(String orderid) {
	this.orderid = orderid;
}
public String getCxr() {
	return cxr;
}
public void setCxr(String cxr) {
	this.cxr = cxr;
}
public String getStime() {
	return stime;
}
public void setStime(String stime) {
	this.stime = stime;
}
public String getBb() {
	return bb;
}
public void setBb(String bb) {
	this.bb = bb;
}
public String getOnecxtime() {
	return onecxtime;
}
public void setOnecxtime(String onecxtime) {
	this.onecxtime = onecxtime;
}
public String getCxerror() {
	return cxerror;
}
public void setCxerror(String cxerror) {
	this.cxerror = cxerror;
}
public String getErrortest() {
	return errortest;
}
public void setErrortest(String errortest) {
	this.errortest = errortest;
}
  
  


  
  
  
}
