...
public class erp_userloginController {
	...
	@ResponseBody
	public String erp_login(String username, String password,
		...
		if (pd != null && !pd.equals("")) {
			int login_type = 0;
			if (pd.get("login_type") != null
					&& !pd.get("login_type").equals("")) {
				login_type = Integer.parseInt(pd.get("login_type").toString());
			}
			if (login_type != 1) {
	...
	public String erp_outlogin(HttpServletRequest request) {
		HttpSession session = request.getSession();
		// 清除Session
		session.invalidate();
