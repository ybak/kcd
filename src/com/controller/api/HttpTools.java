...
public class HttpTools {
	...
	public static String getIPbelongAddress(String ip) {
		...
		try {
			// 淘宝提供的服务地址
			String context = call("http://ip.taobao.com/service/getIpInfo.php?ip="
					+ ip);
	...
	public static JSONObject httpsRequest(String requestUrl,
		...
		try {
			 ...
			 * SSLContext.getInstance("SSL", "ttss"); sslContext.init(null, tm,
	...
	// public static TtMap httpClientPostmss(String urlParam,
	// Map<String, Object> params, String charset, TtMap headers) {
	 ...
	 */
	public static String httpClientPost_String(String urlParam, String params,
			String charset, TtMap headers) {
		StringBuffer resultBuffer = null;
		CloseableHttpClient client = HttpClientBuilder.create().setProxy(null)
				.build();
		...
		try {
			...
			if (headers != null && headers.size() > 0) {
				for (Map.Entry<String, String> entryHeader : headers.entrySet()) {
					httpPost.addHeader(entryHeader.getKey(),
			...
			for (Map.Entry<String, Object> entry : params.entrySet()) {
				...
				try {
					sbParams.append(URLEncoder.encode(
							String.valueOf(entry.getValue()), charset));// todo,这里会自动urlencode
	...
	public static String httpClientGet(String url, String param,
		...
		try {
			...
			URL realUrl = new URL(urlNameString);// URLEncoder.encode(param,
													// "UTF-8")
			// 打开和URL之间的连接
